...
def main(n, M, D, Lambda,C, y ,Features, ratio_data,Random_seed):
    ...
    ylabel("log(1+MSE)")
    title("Errors plot Ridge with N fixed")
    legend()
...
for Random_seed in Liste_seeds:
    Beta_norm, Train_error, Test_error, Global_error = main(n, M, D, Lambda,C, y, Features, r, Random_seed)
...
clf()
fill_between(liste_P, Beta_norm_min, Beta_norm_max, color='blue', alpha=0.3, label='Beta_norm_std')
fill_between(liste_P, Train_error_min, Train_error_max, color='orange', alpha=0.3, label='Train_error_std')
fill_between(liste_P, Test_error_min, Test_error_max, color='green', alpha=0.3, label='Test_error_std')
fill_between(liste_P, Global_error_min, Global_error_max, color='red', alpha=0.3, label='Global_error_std')
plot(liste_P, mean(Liste_Beta_norm,axis=0), color='blue', label = "Beta_norm_mean")
...
xlabel("P/N ratio with P:nb parameters, N: nb points")
ylabel("log(1+MSE)")
title("Mean errors plot Ridge")
legend()
savefig('Graph_error_Ridge_results.png')
